...
Button is the first element of interaction for user and the most basic building block of a component
system. User interacts with a button using mouse, keyboard or a tap on touch screen interface.

> Buttons are the first element of application branding.

## Button - UX Principles

### Types of buttons
At a baseline, buttons have **three distinct flavours**. An applicantion using Clarity Atoms can
build on these three variation to build it own customized buttons. Following are the three variations.
...
### Size
Size of the button should consider all the possibilities with which user would interfact with an
application. Some users with motors skill disorders have trouble interfacting with buttons if they
...
Every button variant must support 4 mandatory states and 1 optional state. The states are as Following
  ...
  1. **Normal & enabled** - The natural visual state of the button when user has not interacted with it.
      Note that - "button should look like a button in a normal state".
...
### Single Line Buttons
Multi-line buttons should be avoided and long-text buttons should be avoided.
...
and as such not part of the Clarity Atoms. Please refer to your products UX guidelines or design system.
